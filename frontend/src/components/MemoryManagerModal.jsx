...
export default function MemoryManagerModal({ isOpen, onClose, chatHistory, windowed = false }) {
  ...
  const fetchMemories = async () => {
    try {
      ...
      const nextMemories = Array.isArray(res.data)
        ? res.data
        : Array.isArray(res.data?.memories)
          ? res.data.memories
          : [];
      setMemories(nextMemories);
      setSystemMeta(res.data?.system || null);
    } catch (err) {
  ...
  const filteredMemories = safeMemories.filter(memory => {
    const name = String(memory?.name || '').toLowerCase();
    const preview = String(memory?.preview || '').toLowerCase();
    const category = String(memory?.category || '').toLowerCase();
    if (!normalizedSearch) return true;
    return name.includes(normalizedSearch) || preview.includes(normalizedSearch) || category.includes(normalizedSearch);
  });
  ...
  return (
    ...
    >
      <motion.div
        className={windowed ? 'bg-white rounded-[28px] w-full h-full shadow-none overflow-hidden flex flex-col' : 'bg-white rounded-3xl w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col h-[80vh]'}
                  ...
                  {systemMeta && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                      <div className="rounded-2xl border border-purple-100 bg-purple-50/70 px-4 py-3">
                        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-purple-400">Long-Term</div>
                        <div className="mt-1 text-2xl font-bold text-gray-800">{systemMeta.longTermCount ?? 0}</div>
                      </div>
                      <div className="rounded-2xl border border-blue-100 bg-blue-50/70 px-4 py-3">
                        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-blue-400">Short-Term</div>
                        <div className="mt-1 text-2xl font-bold text-gray-800">{systemMeta.shortTermTopicCount ?? 0}</div>
                      </div>
                      <div className="rounded-2xl border border-amber-100 bg-amber-50/70 px-4 py-3">
                        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-amber-500">Sleep Queue</div>
                        <div className="mt-1 text-2xl font-bold text-gray-800">{systemMeta.pendingQueueSize ?? 0}</div>
                      </div>
                      <div className="rounded-2xl border border-gray-200 bg-gray-50/80 px-4 py-3">
                        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-gray-400">Last Sleep</div>
                        <div className="mt-1 text-xs font-semibold text-gray-700 break-all">
                          {systemMeta.lastSleepCycleAt || 'Not run yet'}
                        </div>
                      </div>
                    </div>
                  )}
                      ...
                      {filteredMemories.map(memory => (
                        <div key={memory.name} className="group p-5 bg-white border-2 border-gray-50 rounded-2xl hover:border-purple-200 hover:shadow-xl transition-all flex flex-col gap-3 relative">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            {memory.category && (
                              <span className="px-2 py-1 rounded-full bg-purple-50 text-purple-600 text-[10px] font-bold uppercase tracking-wide">
                                {memory.category}
                              </span>
                            )}
                            {memory.auto && (
                              <span className="px-2 py-1 rounded-full bg-orange-50 text-orange-600 text-[10px] font-bold uppercase tracking-wide">
                                auto
                              </span>
                            )}
