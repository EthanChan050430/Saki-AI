...
import { X, FileText, Trash2, Download, Eye, Edit2, Save, File as FileIcon, Search, ArrowLeft, CheckCircle2, Folder, ChevronRight, CornerLeftUp } from 'lucide-react';
import { motion } from 'framer-motion';
import axios from 'axios';
import { BACKEND_URL } from '../utils/backendUrl';
import { modalBackdropMotion, modalPanelMotion } from '../utils/modalMotion';

export default function FileManagerModal({ isOpen, onClose, onSelect, windowed = false }) {
  ...
  const handleDelete = async (file) => {
    ...
    try {
      await axios.delete(`${BACKEND_URL}/api/files/${encodeURIComponent(file.path)}`);
      fetchFiles();
    } catch (err) {
      alert(t('delete_fail'));
    }
  ...
  const handleDownload = (file) => {
    window.open(`${BACKEND_URL}/api/files/download?path=${encodeURIComponent(file.path)}`);
  };

  const handlePreview = async (file) => {
    try {
      const res = await axios.get(`${BACKEND_URL}/api/files/preview/${encodeURIComponent(file.path)}`);
      setPreviewingFile({ name: file.name, ...res.data });
    } catch (err) {
  ...
  const handleEdit = async (file) => {
    try {
      const res = await axios.get(`${BACKEND_URL}/api/files/preview/${encodeURIComponent(file.path)}`);
      setEditingFile({ name: file.name, path: file.path, content: res.data.content });
    } catch (err) {
      alert(t('read_fail'));
    }
  };
  ...
  return (
    <motion.div
      className={windowed ? 'h-full w-full' : 'fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4'}
      {...(!windowed ? modalBackdropMotion : {})}
      onClick={!windowed ? onClose : undefined}
    >
      <motion.div
        className={windowed ? 'bg-white w-full h-full rounded-[28px] shadow-none relative flex flex-col overflow-hidden' : 'bg-white w-full max-w-4xl h-[80vh] rounded-2xl shadow-2xl relative flex flex-col overflow-hidden'}
            ...
            <>
              <div className="p-4 bg-white sticky top-0 z-10 flex items-center gap-4 border-b">
                ...
                {view === 'files' && currentPath && (
                  <button 
                    onClick={handleGoUp}
                    className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg text-sm transition-colors"
                  >
                ...
                <div className="relative flex-1">
                  ...
                  <input 
                    type="text" 
                    placeholder={view === 'files' ? (currentPath ? `/${currentPath}` : t('search_file_placeholder')) : t('search_trash_placeholder')}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {view === 'files' ? (
                      filteredFiles.map(file => (
                        <div key={file.path} className="group p-4 bg-white border border-gray-100 rounded-2xl hover:border-blue-200 hover:shadow-md transition-all flex items-center gap-3">
                          <div 
                            className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors cursor-pointer ${file.isDirectory ? 'bg-amber-50 text-amber-500 group-hover:bg-amber-100' : 'bg-gray-50 text-gray-400 group-hover:bg-blue-50 group-hover:text-blue-500'}`}
                            onClick={() => file.isDirectory && handleOpenFolder(file.path)}
                          >
                            {file.isDirectory ? <Folder size={20} /> : <FileText size={20} />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p 
                              className={`text-sm font-medium text-gray-700 truncate ${file.isDirectory ? 'cursor-pointer hover:text-amber-600' : ''}`}
                              onClick={() => file.isDirectory && handleOpenFolder(file.path)}
                            >
                              {file.name}
                            </p>
                            <p className="text-[10px] text-gray-400">{file.isDirectory ? t('folder') : file.size} • {file.time}</p>
                          </div>
                          <div className="flex items-center gap-1 opacity-100 group-hover:opacity-100 transition-opacity">
                            ...
                            <div className={`flex items-center gap-1 ${onSelect ? 'opacity-0 group-hover:opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                              {file.isDirectory ? (
                                <button onClick={() => handleOpenFolder(file.path)} className="p-1.5 hover:bg-amber-100 text-amber-500 rounded-lg transition-colors" title={t('open')}>
