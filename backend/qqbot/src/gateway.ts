...
import { sanitizeQQOutboundText } from "./utils/text-sanitize.js";
...
function filterInternalMarkers(text: string): string {
  ...
  let result = sanitizeQQOutboundText(text).replace(/\[\[[a-z_]+:\s*[^\]]*\]\]/gi, "");
...
interface QueuedMessage {
  ...
  guildId?: string;
  groupOpenid?: string;
  attachments?: Array<{ content_type: string; url: string; filename?: string; asr_refer_text?: string; voice_wav_url?: string }>;
}
...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  // 初始化 API 配置（markdown 和沙箱支持）
  initApiConfig({
    markdownSupport: account.markdownSupport,
    sandbox: account.sandbox === true,
  });
  log?.info(`[qqbot:${account.accountId}] API config: markdownSupport=${account.markdownSupport === true}, sandbox=${account.sandbox === true}`);
  ...
  const connect = async () => {
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        try {
          if (event.type === "c2c" && event.senderId) {
            ...
            log?.debug?.(`[qqbot:${account.accountId}] Sent input notify to ${event.senderId}`);
          }
        } catch (err) {
          const errMsg = String(err);
          if (
            errMsg.includes("Input notify transient failure") ||
            ...
            log?.warn?.(`[qqbot:${account.accountId}] Input notify skipped: ${errMsg}`);
          } else {
        ...
        if (event.attachments?.length) {
          ...
          for (const att of event.attachments) {
            ...
            const qqNativeTranscript = typeof att.asr_refer_text === "string" ? att.asr_refer_text.trim() : "";
            if (isVoice && qqNativeTranscript) {
              log?.info(`[qqbot:${account.accountId}] Voice attachment: ${att.filename}, using QQ native transcript`);
              voiceTranscripts.push(qqNativeTranscript);
              continue;
            }
        ...
        const sttHint = hasSTT
          ? `\n7. 用户发送的语音消息会自动转录为文字；如果 QQ 平台已附带识别文本，系统会优先使用`
          : `\n7. 如果 QQ 平台已附带语音识别文本，系统会优先使用；否则在未配置 STT 时无法继续自动转录语音消息`;
        const voiceSection = `
...
`;
          ...
          const dispatchPromise = pluginRuntime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            dispatcherOptions: {
              ...
              deliver: async (payload: { text?: string; mediaUrls?: string[]; mediaUrl?: string }, info: { kind: string }) => {
                ...
                let replyText = sanitizeQQOutboundText(payload.text ?? "");
                ...
                if (mediaTagMatches.length > 0) {
                  ...
                  while ((match = mediaTagRegexWithIndex.exec(replyText)) !== null) {
                    ...
                    if (textBefore) {
                      const cleanTextBefore = filterInternalMarkers(textBefore);
                      if (cleanTextBefore) {
                        sendQueue.push({ type: "text", content: cleanTextBefore });
                      }
                    }
                  ...
                  if (textAfter) {
                    const cleanTextAfter = filterInternalMarkers(textAfter);
                    if (cleanTextAfter) {
                      sendQueue.push({ type: "text", content: cleanTextAfter });
                    }
                  }
      ...
      ws.on("open", () => {
        ...
        startBackgroundTokenRefresh(account.appId, account.clientSecret, {
          log: log as { info: (msg: string) => void; warn?: (msg: string) => void; error: (msg: string) => void; debug?: (msg: string) => void },
        });
      });
