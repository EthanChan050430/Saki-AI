...
let currentSandboxMode = false;

/**
 * 初始化 API 配置
 * @param options.markdownSupport - 是否支持 markdown 消息（默认 false，需要机器人具备该权限才能启用）
 * @param options.sandbox - 是否使用沙箱环境（默认 false）
 */
export function initApiConfig(options: { markdownSupport?: boolean; sandbox?: boolean }): void {
  currentMarkdownSupport = options.markdownSupport === true;
  currentSandboxMode = options.sandbox === true;
}
...
export function getApiBase(): string {
  return currentSandboxMode ? SANDBOX_API_BASE : API_BASE;
}
...
function buildMessageBody(
  ...
  const cleanContent = sanitizeQQOutboundText(content);
  if (!cleanContent) {
    throw new Error("QQ message content is empty after filtering private reasoning");
  }
...
function buildProactiveMessageBody(content: string): Record<string, unknown> {
  ...
  if (currentMarkdownSupport) {
    return { markdown: { content: cleanContent }, msg_type: 2 };
  } else {
    return { content: cleanContent, msg_type: 0 };
  }
}
